#!/usr/bin/env python3
"""
将 OpenViking traffic snapshot + 本地 insights 快照 同步到飞书表 OpenViking 漏斗(日)。

说明:
- 访客/克隆者来自 GitHub Traffic
- Stars日增量 / Contributors日增量 / Stars总数 / 贡献者总数 来自本地 insights 快照
- 因此历史日期是否能完整算出，取决于对应日期是否存在 insights_YYYY-MM-DD.json
"""
...
from openviking_traffic_utils import build_funnel_rows, load_latest_insights_by_date, load_traffic_snapshot
...
def main() -> int:
    ...
    table_id = os.getenv("LARK_OPENVIKING_FUNNEL_TABLE_ID", "")
    if not base_token or not table_id:
        print("缺少 .env 配置: LARK_BASE_TOKEN / LARK_OPENVIKING_FUNNEL_TABLE_ID")
        return 2

    snapshot_path = resolve_snapshot_path(sys.argv[1] if len(sys.argv) > 1 else None)
    snapshot = load_traffic_snapshot(snapshot_path)
    insights_by_date = load_latest_insights_by_date(ROOT / "data")
    rows = build_funnel_rows(snapshot, insights_by_date)
