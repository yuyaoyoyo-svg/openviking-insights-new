#!/usr/bin/env python3
"""
创建/补齐 OpenViking Traffic(日) 表，并将表 ID 回写到 .env。
"""
...
TABLE_NAME = "OpenViking Traffic(日)"
ENV_KEY = "LARK_OPENVIKING_TRAFFIC_TABLE_ID"
...
FIELD_DEFS = [
    ...
    {"type": "number", "name": "总浏览量(14d)"},
    {"type": "number", "name": "总访客数(14d)"},
    {"type": "number", "name": "总克隆次数(14d)"},
    {"type": "number", "name": "总克隆者数(14d)"},
    {"type": "text", "name": "Top Referrers 摘要"},
    {"type": "text", "name": "Top Paths 摘要"},
...
def create_table(base_token):
    ...
    if not table_id:
        raise RuntimeError("创建 OpenViking Traffic(日) 表后未找到新表 ID")
    return table_id
